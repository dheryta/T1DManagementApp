...
public class BGCheckingSchedule extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ActivityArrayAdapter adapter = new ActivityArrayAdapter(this,
				getModel(), R.layout.activity_bgchecking_schedule);
		...
		case R.id.menuDone:
			...
			if (appContext.getDbHandler().insertActivitySchedule(schedule) == 1){
				for(int i=0;i<schedule.size();i++){
					...
					intent.putExtra("ID", 11+i+1);	
					intent.setAction("BGCheckingAlarmFor"+schedule.get(i).get_SubType());
					
				    commonMethods.setAlarm(calendar.getTimeInMillis(), intent , BGCheckingSchedule.this, commonMethods.REPEAT.get(0));
					}
					}
						
			}
			else
				Toast.makeText(BGCheckingSchedule.this, "T1DM says, oops error occurred", Toast.LENGTH_LONG).show();
	...
	private List<Model> getModel() {
		...
		List<Model> list = appContext.getDbHandler().getAlarms(CommonMethods.ACTIVITY_BGCHECKING);
		if (list.size() == 0)
