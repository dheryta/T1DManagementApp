...
public class SleepSchedule extends ListActivity {
	private CommonMethods commonMethods = new CommonMethods();
	private T1DMApplication appContext;
	
	private String[] values = new String[] { commonMethods.SUBTYPE_NOON,
			commonMethods.SUBTYPE_NIGHT, commonMethods.SUBTYPE_EXTRAS };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ActivityArrayAdapter adapter = new ActivityArrayAdapter(this,
				getModel(), R.layout.activity_sleep_schedule);
		...
		case R.id.menuDone:
			...
			if (appContext.getDbHandler().insertActivitySchedule(schedule) == 1){
				for(int i=0;i<schedule.size();i++){
					...
					intent.putExtra("ID", 21+i+1);
					intent.setAction("SleepAlarmFor"+schedule.get(i).get_SubType());
					
				    commonMethods.setAlarm(calendar.getTimeInMillis(), intent , SleepSchedule.this, commonMethods.REPEAT.get(0));
					}
					}
				...
				Toast.makeText(SleepSchedule.this, "T1DM says, oops error occurred", Toast.LENGTH_LONG).show();
	...
	private List<Model> getModel() {
		List<Model> list = appContext.getDbHandler().getAlarms(CommonMethods.ACTIVITY_SLEEP);
		if (list.size() == 0)
